...
namespace faceoff.Core
{
    ...
    public class FaceOffService
    {
        ...
        private FaceRecognitionModel RecognitionModel4 = FaceRecognitionModel.Recognition04;
        private static readonly string LargePersonGroupId = Guid.NewGuid().ToString();
        private static FaceClient client;
             
        //Private FaceClient = Authenticate(Endpoint, SubscriptionKey);
        public FaceOffService(IConfiguration configuration)
        {
           
            _configuration = configuration.GetRequiredSection("FaceAPI");
